...
type
  ...
  private
    ...
    procedure RequeryAnakU21;
    //function GetSisaPensiun(TglLahir: TDateTime; MaxPensiun: Integer): String;
  public
    { Public declarations }
  end;

var
  frmAnakU21: TfrmAnakU21;

Procedure MonitoringAnakU21Dlg;
...
Procedure MonitoringAnakU21Dlg;
begin
  if (frmAnakU21=nil) then frmAnakU21:= TfrmAnakU21.Create(Nil);
  frmAnakU21.Show;
  frmAnakU21:= nil;
end;

procedure TfrmAnakU21.CreateTreeMenu;
...
function TfrmAnakU21.GetFilterSQL: String;
...
procedure TfrmAnakU21.RequeryAnakU21;
var S, F: String;
begin
  qryPensiun.Close;
  ...
  S:= 'select * from ViewTerTanggung Where (not Aktif_Kd in(3,4,5,6)) ' +
      'and ((Hub_Kd<>''SU'') and (Hub_Kd<>''IS'')) ' + //and (IsTanggung=1) ' +
...
procedure TfrmAnakU21.FormCreate(Sender: TObject);
begin
  BaruBuka:= True;
...
procedure TfrmAnakU21.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  Action:= caFree;
end;

procedure TfrmAnakU21.TVWChange(Sender: TObject; Node: TTreeNode);
begin
...
procedure TfrmAnakU21.btnCloseClick(Sender: TObject);
begin
  Close;
end;

procedure TfrmAnakU21.qryPensiunBeforeInsert(DataSet: TDataSet);
begin
  Abort;
end;

procedure TfrmAnakU21.qryPensiunAfterOpen(DataSet: TDataSet);
var I: Integer;
begin
...
procedure TfrmAnakU21.txtCariChange(Sender: TObject);
begin
  RequeryAnakU21;
end;

procedure TfrmAnakU21.qryPensiunCalcFields(DataSet: TDataSet);
var Putus: TDateTime;
begin
  if (Dataset.FieldByName('TglPutusTunj').IsNull) then
    Putus:= IncYear(Dataset.FieldByName('TanggalLahir').AsDateTime, 21)
  else Putus:= Dataset.FieldByName('TglPutusTunj').AsDateTime;
  Dataset.FieldByName('TanggalPutus').AsDateTime:= Putus;
...
procedure TfrmAnakU21.btnPrintClick(Sender: TObject);
begin
  DMReports.ShowReportMonitoring(GModule.Path + 'Rpt-Monitoring\MonitoringAnakU21.fr3',
        GModule.Tahun, IntToStr(CoBulan.ItemIndex),
        txtCari.Text, SelectedMenu);
end;

procedure TfrmAnakU21.DesignEdit1Click(Sender: TObject);
begin
  DMReports.DesignReportMonitoring(GModule.Path + 'Rpt-Monitoring\MonitoringAnakU21.fr3',
        GModule.Tahun, IntToStr(CoBulan.ItemIndex),
        txtCari.Text, SelectedMenu);
end;
