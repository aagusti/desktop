...
uses
  ...
  Dialogs, Menus, DB, ADODB, StdCtrls, Buttons, DBCtrls, Grids, DBGrids, DateUtils,
  SMDBGrid, ExtCtrls, ComCtrls, GlobalModule;

type
  TfrmMonitorKGB = class(TForm)
    Splitter2: TSplitter;
    TVW: TTreeView;
    Panel1: TPanel;
    SMDBGrid1: TSMDBGrid;
    ...
    qryPensiunNextKGB: TDateTimeField;
  ...
  private
    ...
    procedure RequeryKGB;
    function GetSisaKGB(TMTMasaKerja: TDateTime): String;
  public
    { Public declarations }
  end;

var
  frmMonitorKGB: TfrmMonitorKGB;

Procedure MonitoringKGBDlg;
...
Procedure MonitoringKGBDlg;
begin
  if (frmMonitorKGB=nil) then frmMonitorKGB:= TfrmMonitorKGB.Create(Nil);
  frmMonitorKGB.Show;
  frmMonitorKGB:= nil;
end;

procedure TfrmMonitorKGB.CreateTreeMenu;
...
function TfrmMonitorKGB.GetFilterSQL: String;
...
procedure TfrmMonitorKGB.RequeryKGB;
var S, F: String;
begin
  qryPensiun.Close;
  ...
  S:= 'select * from ViewPegawai Where (not Aktif_Kd in(3,4,5,6)) ' +
      'and (not TMT_GAJI_POKOK Is Null) AND (YEAR(TMT_GAJI_POKOK) + 2 = ' + GModule.Tahun + ') ';
  if (CoBulan.ItemIndex>0) then
    S:= S + ' and (Month(TMT_GAJI_POKOK)=' + IntToStr(CoBulan.ItemIndex) + ') ';
  if (F<>'') then S:= S + ' and ' + F;
...
function TfrmMonitorKGB.GetSisaKGB(TMTMasaKerja: TDateTime): String;
var Res: Integer;
    dNext: TDateTime;
begin
  Result:= '';
  dNext:= IncYear(TMTMasaKerja, 2);
  Res:= MonthsBetween(Date, dNext);
  if (dNext<Date) then Res:= -1 * Res;
  Result:= FormatFloat('00', Res) + ' bln';
end;

procedure TfrmMonitorKGB.FormCreate(Sender: TObject);
begin
  BaruBuka:= True;
...
procedure TfrmMonitorKGB.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  Action:= caFree;
end;

procedure TfrmMonitorKGB.TVWChange(Sender: TObject; Node: TTreeNode);
begin
...
procedure TfrmMonitorKGB.btnCloseClick(Sender: TObject);
begin
  Close;
end;

procedure TfrmMonitorKGB.qryPensiunBeforeInsert(DataSet: TDataSet);
begin
  Abort;
end;

procedure TfrmMonitorKGB.qryPensiunCalcFields(DataSet: TDataSet);
begin
  if (Dataset.FieldByName('TMT_Gaji_Pokok').IsNull) then
    begin
      Dataset.FieldByName('SisaText').AsString:= 'Invalid'
    end
  else
    begin
      Dataset.FieldByName('NextKGB').AsDateTime:= IncYear(Dataset.FieldByName('TMT_GAJI_POKOK').AsDateTime, 2);
      Dataset.FieldByName('SisaText').AsString:=
        GetSisaKGB(Dataset.FieldByName('TMT_Gaji_Pokok').AsDateTime);
    end;
...
procedure TfrmMonitorKGB.qryPensiunAfterOpen(DataSet: TDataSet);
var I: Integer;
begin
...
procedure TfrmMonitorKGB.txtCariChange(Sender: TObject);
begin
  RequeryKGB;
end;

procedure TfrmMonitorKGB.btnPrintClick(Sender: TObject);
begin
  DMReports.ShowReportMonitoring(GModule.Path + 'Rpt-Monitoring\MonitoringKGB.fr3',
        GModule.Tahun, IntToStr(CoBulan.ItemIndex),
        txtCari.Text, SelectedMenu);
end;

procedure TfrmMonitorKGB.DesignEdit1Click(Sender: TObject);
begin
  DMReports.DesignReportMonitoring(GModule.Path + 'Rpt-Monitoring\MonitoringKGB.fr3',
        GModule.Tahun, IntToStr(CoBulan.ItemIndex),
        txtCari.Text, SelectedMenu);
end;
